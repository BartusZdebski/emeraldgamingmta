...
local function editNPC(thePlayer, commandName, targetID, operation, value)
	if exports.global:isPlayerTrialAdmin(thePlayer) then
		...
		operation = string.lower(tostring(operation))

		if (operation == "type") then
			if not value then
				return outputChatBox("SYNTAX: /" .. commandName .. " [NPC ID] type [New Type]", thePlayer, 75, 230, 10) end

			blackhawk:setElementDataEx(thePed, "npc:type", tonumber(value), true)

			reloadNPC(tonumber(targetID))

			outputChatBox("You have changed NPC #" .. targetID .. "'s type to '" .. tostring(value) .. "'.", thePlayer, 0, 255, 0)
		...
		elseif(operation == "name") then
			...
			blackhawk:setElementDataEx(thePed, "npc:name", tostring(value), true)
			blackhawk:setElementDataEx(thePed, "name", tostring(value), true)

			reloadNPC(tonumber(targetID))

			local thePlayerName = exports.global:getStaffTitle(thePlayer)

			outputChatBox("You have changed NPC #" .. targetID .. "'s name to '" .. tostring(value) .. "'.", thePlayer, 0, 255, 0)
			exports.global:sendMessageToManagers("[INFO] " .. thePlayerName .. " changed NPC #" .. targetID .. "'s name to " .. value)
		end
	end
end
